...
if UseWorldCoast
    ...
    c.coastlat = s(ind,1).Y;
    c.coastlon = s(ind,1).X;
...
% skipped.
if step>1
    nanz = isnan(c.coastlat); 
    ind = flipud(find(nanz)); 
    for k = 1:length(ind)
        appendlat = [NaN(step,1);c.coastlat(ind(k)+1:end)];
        appendlon = [NaN(step,1);c.coastlon(ind(k)+1:end)];
        c.coastlat(ind(k):ind(k)+length(appendlat)-1) = appendlat; 
        c.coastlon(ind(k):ind(k)+length(appendlat)-1) = appendlon; 
    end
end

land = inpolygon(lon,lat,c.coastlon(1:step:end),c.coastlat(1:step:end));
