...
% For Nimbus
saveloc = '/campaign/RC-EE1246A-001/peter-phoebe/output';
AIRSdataloc = '/campaign/RC-EE1246A-001/peter-phoebe/AIRS/3d_airs/';
...
for year = 2012:1:2012
    for day = 183:183
        ...
        savelocation = fullfile(saveloc,num2str(year));
        filename = fullfile(savelocation, date);
        
        dayParams = fullAquis(year, month, day, AIRSdataloc);
...
function [bigParam] = fullAquis(year, month, day, AIRSdataloc)
    ...
    for granule = 240
        ...
        if granule == 240
            file = strcat(AIRSdataloc,string(year),"/",dayofyear,"/airs_", string(year), "_", dayofyear, "_", sprintf("%03d",granule),".nc")
            file2 = strcat(AIRSdataloc,string(year),"/",sprintf("%03d", day+1),"/airs_", string(year), "_", sprintf("%03d", day+1), "_", sprintf("%03d",1),".nc")
            
        else
    
            file = strcat(AIRSdataloc,string(year),"/",dayofyear,"/airs_", string(year), "_", dayofyear, "_", sprintf("%03d",granule),".nc");
            file2 = strcat(AIRSdataloc,string(year),"/",dayofyear,"/airs_", string(year), "_", dayofyear, "_", sprintf("%03d",granule+1),".nc");
        end % if granule = 240

        if exist(file) 
            if exist(file2) 

                %[AirOne] = prep_airs_3d(datenum(year, month, day), granule, 'DayNightFlag', true, 'FullDataDir', AIRSdataloc);
				
                [AirOne,Spacing,Error,ErrorInfo,MinorErrorInfo] = prep_airs_3d(datenum(year, month, day), granule, 'DayNightFlag', true);
